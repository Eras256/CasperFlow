...
export default function Marketplace() {
    ...
    const [sortBy, setSortBy] = useState<"amount" | "yield" | "term" | "newest" | "oldest">("amount");
                        ...
                        <div className="flex gap-3">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className="input-glass py-3 px-4 pr-10 text-sm bg-black/40 border border-white/10 text-white rounded-xl appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[var(--flow-cyan)] hover:bg-white/10 transition-colors"
                                    style={{ backgroundImage: 'none' }}
                                >
                                    <option value="amount" className="bg-[#0B1221] text-white">Sort by Amount</option>
                                    <option value="newest" className="bg-[#0B1221] text-white">Sort by Newest</option>
                                    <option value="oldest" className="bg-[#0B1221] text-white">Sort by Oldest</option>
                                    <option value="yield" className="bg-[#0B1221] text-white">Sort by Yield</option>
                                    <option value="term" className="bg-[#0B1221] text-white">Sort by Term</option>
                                </select>
                                {/* Custom arrow icon overlay */}
                                <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--flow-text-muted)]">
